...
static void add_wait(struct task_struct ** wait_address, select_table * p) {
    int i;
    if (!wait_address) return;
    for (i = 0; i < p->nr; i++) {
        if (p->entry[i].wait_address == wait_address) return;
    }
...
static void free_wait(select_table * p) {
    ...
    for (i = 0; i < p->nr; i++) {
        ...
        if (!*tpp) printk("free_wait: NULL");
        if ((*tpp = p->entry[i].old_task)) (**tpp).state = 0;
    }
    p->nr = 0;
}
...
static struct tty_struct * get_tty(struct m_inode * inode) {
    int major, minor;
    if (!S_ISCHR(inode->i_mode)) return NULL;
    if ((major = MAJOR(inode->i_zone[0])) != 5 && major != 4) return NULL;
    if (major == 5) {
        minor = current->tty;
    } else {
...
int do_select(fd_set in, fd_set out, fd_set ex,
    ...
    for (i = 0; i < NR_OPEN; i++, mask >>= 1) {
        if (!(mask & 1)) continue;
        if (!current->filp[i]) return -EBADF;
        if (!current->filp[i]->f_inode) return -EBADF;
        if (current->filp[i]->f_inode->i_pipe) continue;
        if (S_ISCHR(current->filp[i]->f_inode->i_mode)) continue;
        if (S_ISFIFO(current->filp[i]->f_inode->i_mode)) continue;
        return -EBADF;
    }
repeat:
    wait_table.nr = 0;
...
int sys_select( unsigned long *buffer ) {
    ...
    if (!i && (current->signal & ~current->blocked)) return -EINTR;
    return i;
}
